...
Load technical markdown settings and a custom function for printing only what is needed through **printn()** function.

```{r include=FALSE}


knitr::opts_chunk$set(
  message = TRUE,  # suppress package startup and other messages
  warning = FALSE,  # suppress warnings
  echo    = TRUE,   # still show code unless you turn it off per chunk
  results = "hide" 
)
...
printn <- function(x) {
  ...
  # Using message to show even if results="hide"
...
```{r}
library(FBMS)
# Parameters for parallel runs which you might choose specifically for your machine
runs <- 1  #use e.g. 8 on your machine
cores <- 1 #use e.g. 8 on your machine
...
Vast, depending on allowed nonlinear functions ${G}$. Includes:\
...
Custom functions can be added to ${G}$.
...
P(\boldsymbol{\beta} | g) &\sim N_{|M|}(\boldsymbol{0}, g \cdot \phi {J}_n(\boldsymbol{\beta})^{-1}) \\
...
**Jeffreys prior**:\
$$
P(\phi | M) = \phi^{-1}, \quad P(\beta_0, \boldsymbol{\beta} | M) = |{J}_n(\beta_0, \boldsymbol{\beta})|^{\frac{1}{2}}
$$
...
*Parameter priors available in the FBMS package with hyperparameters and applicability.* $p_{M}$ is the number of predictors excluding the intercept. G stands for only "gaussian" family, GLM includes additionally the "binomial", "poisson", and "gamma" families. Hyperparameters $a, b, \rho, s, v, k$ are as defined for tCCH priors; others use specific parameters as noted. Parameters in italics (e.g., $a, s, g$) denote user-specified values.
...
- Embeds MJMCMC in a genetic algorithm with populations ${S}_1, {S}_2, \dots, {S}_{T_{max}}$.\
...
### Example of BGNLM: third Kepler's Law recovery
...
#note that with runs = 1, convergence is not likely here, change runs and cores to improve.
...
$$ **Data**:\
$$
X \sim {MVN}(0, \textbf{I}_{20}), \quad y = X^T \boldsymbol{\beta} + \varepsilon, \quad \varepsilon \sim {N(0, 1)}
$$
...
```{r}
result.lin <- fbms(formula = Y ~ 1 + ., data = df, beta_prior = list(type = "g-prior", g = 1000), method = "mjmcmc", N = 5000)
```
...
- ${K}$: Indexes of $\mathbf{F}_0 \cup \mathbf{F}_1 \cup \mathbf{F}_2$\
- $\rho_k(x)$: Polynomial term from $\{\mathbf{F}_0 \cup \mathbf{F}_1 \cup \mathbf{F}_2\}_k$

$$
\begin{aligned}
Y_i | \mu_i, \phi &\sim \mathfrak{f}(y|\mu_i,\phi), \quad i = 1, \dots, n \\
\mathsf{h}(\mu_i) &= \beta_0 + \sum_{j=1}^{p} \sum_{k \in {K}} \gamma_{jk} \beta_{jk} \rho_k(x_{ij})
\end{aligned}
$$ - Total models: $2^{p \times |{K}|}$

### Example

**True Model**:\
$$
y = {x_1^{0.5} + x_1 + x_3^{-0.5} + x_3^{-0.5} * \log(x_3) + x_{4} + x_5^{-1} + \log(x_6) + x_8 + x_{10} + \epsilon}, \quad \epsilon \sim {N(0, 1)}
$$ **Dimensions**: $n = 250, p = 12$ also covariates from the previous example on linear model.

**Prepare the parameters**

```{r}

df$Y <- p05(df$X1) + df$X1 + pm05(df$X3) + p0pm05(df$X3) + df$X4 + pm1(df$X5) + p0(df$X6) + df$X8 + df$X10 + rnorm(dim(df)[1])
