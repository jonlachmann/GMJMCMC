...
gaussian.loglik.gap.open.prior = function(y, x, model, complex, params){
 ...
 #   ret <- (-(mod$deviance + 2 * log(length(y)) * (mod$rank - 
 #                                                      1) - 2 * log(params$r) * (sum(complex$oc))))/2
     
    # logarithm of marginal likelihood
    mloglik <- -mod$deviance/2 +  log(length(y)) * (mod$rank - 1) 
              
   # logarithm of model prior
       #lp <-  log(params$r) * (sum(complex$depth) + 0.2 * sum(complex$oc))
       lp <-  log(params$r) * (sum(complex$depth * complex$oc))
       #   lp <-   log(params$r) * (sum(complex$oc))
