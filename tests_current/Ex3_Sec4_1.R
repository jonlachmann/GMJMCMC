...
if (run.parallel) {
  ...
  set.seed(1234)
  result_parallel2=fbms(data=df,loglik.pi=gaussian.loglik.g,transforms=transforms,beta_prior = list(type = "g-prior", alpha = max(n,p^2)),
                        probs=probs,params=params,
                        method="gmjmcmc.parallel",
                        P=50,N=1000,N.final=1000,runs=10,cores=10)
  ...
  set.seed(123456)
  result_parallel3=fbms(data=df,loglik.pi=gaussian.loglik.g,transforms=transforms,beta_prior = list(type = "g-prior", alpha = max(n,p^2)),
                        probs=probs,params=params,
                        method="gmjmcmc.parallel",
                        P=50,N=1000,N.final=1000,runs=10,cores=10)
