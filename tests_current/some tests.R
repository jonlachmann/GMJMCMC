...
for (family in families) {
  ...
  for (prior in priors) {
    ...
    # Run the model
    tryCatch({
      result <- fbms.mlik.master2(data$y, data$x, model = c(T, rep(TRUE, ncol(data$x) - 1)), 
                                 complex = list(oc = 1), params = params)
      result.null <- fbms.mlik.master2(data$y, data$x, model = c(T, T, rep(FALSE, ncol(data$x) - 2)), 
                                      complex = list(oc = 1), params = params)
